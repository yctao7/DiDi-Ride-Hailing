...
class loc_table(object):
    def __init__(self, load_file=None):
        ...
        self.max_lng = self.grids[['lng1', 'lng2', 'lng3', 'lng4', 'lng5', 'lng6']].max().max()
        self.min_lng = self.grids[['lng1', 'lng2', 'lng3', 'lng4', 'lng5', 'lng6']].min().min()
        self.max_lat = self.grids[['lat1', 'lat2', 'lat3', 'lat4', 'lat5', 'lat6']].max().max()
        self.min_lat = self.grids[['lat1', 'lat2', 'lat3', 'lat4', 'lat5', 'lat6']].min().min()
        ...
        self.table_table = [[None] * self.col_num] * self.row_num
    ...
    def create_table_table(self):
        ...
        for i in range(self.row_num):
            for j in range(self.col_num):
                self.table_table[i][j] = [[0.0] * self.time_num] * self.date_num
...
class Agent(object):
    ...
    def update_param(self, batch):
        ...
        for i in range(batch.shape[0]):
            ...
            if next_x < 0 or next_x >= self.table.col_num:
                continue
            if next_y < 0 or next_y >= self.table.row_num:
                continue
        ...
        print('MSE between updation:', MSE.item(), 'Value sample:', predict_value.item())
